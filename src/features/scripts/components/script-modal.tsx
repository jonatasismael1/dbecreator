...
import { BrainCircuit, Check, FileText, Loader2, Sparkles, Wand2, X } from 'lucide-react'
import { z } from 'zod'
...
export function ScriptModal({
  ...
  // Deby AI script generation from prompt
  const [debyPrompt, setDebyPrompt] = useState('')
  const [debyGenerating, setDebyGenerating] = useState(false)
  const [debyGenerateError, setDebyGenerateError] = useState<string | null>(null)
  const [debyPreview, setDebyPreview] = useState<{
    title: string
  ...
  const handleGenerateScript = async () => {
    if (!debyPrompt.trim()) return
    setDebyGenerating(true)
    setDebyGenerateError(null)
    setDebyPreview(null)
    try {
      const result = await debyService.generateScript(debyPrompt.trim())
      setDebyPreview(result)
    } catch {
      setDebyGenerateError('Não foi possível gerar o roteiro. Tente novamente.')
    } finally {
      setDebyGenerating(false)
    }
  }

  const applyDebyGeneratedScript = () => {
    if (!debyPreview) return
    setValue('title', debyPreview.title, { shouldDirty: true, shouldValidate: true })
    setValue('hook', debyPreview.hook, { shouldDirty: true, shouldValidate: true })
    setValue('body', debyPreview.body, { shouldDirty: true, shouldValidate: true })
    setValue('cta', debyPreview.cta, { shouldDirty: true, shouldValidate: true })
    setDebyPreview(null)
    setDebyPrompt('')
  }
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98, y: 24 }} className="pointer-events-none fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-4">
            <div className="pointer-events-auto flex max-h-[94dvh] w-full flex-col overflow-hidden rounded-t-2xl border border-dbe-border bg-dbe-navy shadow-2xl sm:max-h-[92vh] sm:max-w-3xl sm:rounded-2xl">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 overflow-y-auto p-4 sm:space-y-5 sm:p-6">
                ...
                {/* Deby AI script generation */}
                {!script && (
                  <div className="rounded-xl border border-dbe-green/20 bg-dbe-green/5 p-4 mb-2 space-y-3">
                    <div className="flex items-center gap-2">
                      <Sparkles className="h-4 w-4 text-dbe-green" />
                      <span className="text-sm font-semibold text-dbe-green">Criar roteiro com a Deby AI</span>
                    </div>
                    <textarea
                      value={debyPrompt}
                      onChange={(e) => setDebyPrompt(e.target.value)}
                      disabled={debyGenerating}
                      rows={3}
                      placeholder="Descreva o roteiro que você quer criar. Ex: crie um roteiro para um Reels sobre clareamento dental com gancho forte, tom educativo e CTA para agendar avaliação."
                      className={`${inputClass} resize-none`}
                    />
                    {debyGenerateError && (
                      <p className="text-xs text-dbe-red">{debyGenerateError}</p>
                    )}
                    <button
                      type="button"
                      onClick={handleGenerateScript}
                      disabled={debyGenerating || !debyPrompt.trim()}
                      className="flex w-full items-center justify-center gap-2 rounded-lg border border-dbe-green/30 bg-dbe-green/10 px-4 py-2.5 text-sm font-semibold text-dbe-green transition-all hover:bg-dbe-green/20 disabled:opacity-40"
                    >
                      {debyGenerating ? (
                        <><Loader2 className="h-4 w-4 animate-spin" /> Gerando roteiro...</>
                      ) : (
                        <><Sparkles className="h-4 w-4" /> Gerar com Deby AI</>
                      )}
                    </button>

                    {/* Preview */}
                    {debyPreview && (
                      <div className="mt-1 space-y-3 rounded-lg border border-dbe-green/30 bg-black/20 p-3">
                        <p className="text-[11px] font-bold uppercase tracking-wider text-dbe-green">Prévia da Deby AI</p>
                        <div className="space-y-2 text-xs">
                          <div>
                            <span className="font-semibold text-dbe-muted">Título: </span>
                            <span className="text-dbe-text">{debyPreview.title}</span>
                          </div>
                          <div>
                            <span className="font-semibold text-dbe-muted">Pilar sugerido: </span>
                            <span className="text-dbe-text">{debyPreview.pillar_suggestion}</span>
                          </div>
                          <div>
                            <span className="font-semibold text-amber-400">Gancho: </span>
                            <span className="text-dbe-text line-clamp-3">{debyPreview.hook}</span>
                          </div>
                          <div>
                            <span className="font-semibold text-dbe-blue">Desenvolvimento: </span>
                            <span className="text-dbe-text line-clamp-3">{debyPreview.body}</span>
                          </div>
                          <div>
                            <span className="font-semibold text-dbe-green">CTA: </span>
                            <span className="text-dbe-text">{debyPreview.cta}</span>
                          </div>
                        </div>
                        <div className="flex gap-2 pt-1">
                          <button
                            type="button"
                            onClick={() => setDebyPreview(null)}
                            className="flex-1 rounded-md border border-dbe-border px-3 py-2 text-xs font-medium text-dbe-muted transition-colors hover:text-dbe-text"
                          >
                            Descartar
                          </button>
                          <button
                            type="button"
                            onClick={applyDebyGeneratedScript}
                            className="flex flex-1 items-center justify-center gap-1.5 rounded-md border border-dbe-green/30 bg-dbe-green/10 px-3 py-2 text-xs font-semibold text-dbe-green transition-all hover:bg-dbe-green/20"
                          >
                            <Check className="h-3.5 w-3.5" />
                            Confirmar e preencher roteiro
                          </button>
                        </div>
                      </div>
                    )}
